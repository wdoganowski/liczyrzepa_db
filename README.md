...
```bash
liczyrzepa-api$ sam local invoke LiczyrzepaAPIFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
liczyrzepa-api$ sam local start-api
liczyrzepa-api$ curl http://localhost:3000/
```
...
```bash
liczyrzepa-api$ sam logs -n LiczyrzepaAPIFunction --stack-name liczyrzepa-api --tail
```
...
```bash
aws cloudformation delete-stack --stack-name liczyrzepa-api
```
